...
During the above computation, dada2 was run and all ASVs are detected. The result is an object of class EvoTraceR, which is populated by 5 fields: dada2 is a list which contains the percentage of chimeras found by dada2 and a dataframe
